...
//                log.info("emailCheckDto : "+emailCheckDto);
    ...
    // 이메일발송 저장 시작 -> 예약일 경우 배치돌면서 발송한다 + 예약발송일 경우 첨부파일은 받지 않음, 즉시발송시작
...
//        log.info("emailSendDto : "+emailSendDto);
        ...
        if(!Utils.isValidEmail(emailSendDto.getEmEmailSend())) {
            log.error("이메일주소 형식과 맞지 않습니다. 발송자 이메일을 확인해주시길 바랍니다. 입력하신 이메일 : "+emailSendDto.getEmEmailSend());
            return ResponseEntity.ok(res.fail(ResponseErrorCode.KO104.getCode(),ResponseErrorCode.KO104.getDesc() +" " +
                    "발송자 이메일을 확인해주시길 바랍니다. 입력하신 이메일 : "+emailSendDto.getEmEmailSend()));
        }
...
//            int emSendAllCount = kokonutUserEmailFieldDtos.size();
...
//            saveEmail.setEmSendAllCount(emSendAllCount);
            ...
            // 즉시발송 서비스 호출
//            emailSendAction(emailSendDto, jwtFilterDto);
    ...
    // 이메일 발송시작
//    public String emailSendAction(String fieldName, String emTitle, String content) throws Exception {
...
//    }
    ...
    // 이메일발송 호출
    public ResponseEntity<Map<String, Object>> kokonutSendEmail(EmailSendDto emailSendDto, JwtFilterDto jwtFilterDto) throws Exception {
        log.info("sendEmailService 호출");
