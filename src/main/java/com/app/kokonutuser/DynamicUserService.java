...
//			log.info("존재하는 테이블");
			...
			CompanyTableColumnNameSearch companyTableColumnNameSearch = null;
			...
			if(!ctNameStatus.equals("")) {
				companyTableColumnNameSearch = companyTableColumnInfoRepository.findByColumnName(ctNameStatus);
			...
			if(!ctEmailStatus.equals("")) {
				...
				securityNames.put("basicEmail", companyTableColumnNameSearch.getCtciDesignation());
...
//					log.info("commentText : "+ Arrays.toString(commentText));
	...
	// 개인정보 검색(신버전) + 이메일발송 회원선택 searchType = "2"일떄
...
//				log.info("resultObject : "+resultObject);
				...
				String code = searchCodes.get(i);
				String value = searchTexts.get(i);
...
//					log.info("value : " + value);
					...
					CompanyTableColumnInfoCheck companyTableColumnInfoCheck = companyTableColumnInfoRepository.findByCheck(ctName, code);
					...
					if (companyTableColumnInfoCheck == null) {
						...
						if(companyTableColumnInfoCheck.getCtciSecuriy().equals("1")) {
							...
							if(awsKmsResultDto == null) {
								awsKmsResultDto = companyDataKeyService.findByCompanyDataKey(cpCode);
							}
						...
						if(companySettingEmailDto != null && emailSendHeader == 0 && code.equals(companySettingEmailDto.getCsEmailCodeSetting())) {
							uniqueDesignation = "지정항목("+companyTableColumnInfoCheck.getCtciDesignation()+")";
							emailSendHeader = 1;
						} else {
							uniqueDesignation = companyTableColumnInfoCheck.getCtciDesignation()+"("+code+")";
						}
...
//			}
			...
			resultQuery.append(selectQuery).append("FROM ").append(ctName).append(" as kokonut ");
					...
					if(key != null) {
						if(!String.valueOf(key).equals("없음")) { // 벨류값이 Null(없음)일 경우 제외
							...
							if(securityWhether.get(i).equals("1")) {
								...
								securityResultValue = Utils.decrypMasking(securityName.get(i), value, awsKmsResultDto.getSecretKey(), awsKmsResultDto.getIvKey());
...
//		Long activityHistoryId;
		...
		// 인증번호 메일전송
		String title = ReqUtils.filter("개인정보열람 파일의 암호가 도착했습니다.");
